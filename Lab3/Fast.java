...
public class Fast {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 0; i < count - 1; ++i) {
            ...
            Map<Double, ArrayList<Point>> slopeMap = new HashMap<>();

            for (int j = i + 1; j < count; ++j) {
                Point pointB = points.get(j);
                double slope = pointA.slopeTo(pointB);

                if (!slopeMap.containsKey(slope)) {
                    slopeMap.put(slope, new ArrayList<>());
                } else {
                    slopeMap.get(slope).add(pointB);
                }
            }

            for (ArrayList<Point> alignedPoints : slopeMap.values()) {
                alignedPoints.add(pointA);

                if (alignedPoints.size() >= 3) {

                    for (Point point : alignedPoints) {
                        renderLine(frame, pointA, point);
                    }
